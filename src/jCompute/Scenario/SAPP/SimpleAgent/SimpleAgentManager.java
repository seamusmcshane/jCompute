...
import jCompute.Scenario.EndEvents.ScenarioAllPredatorsLTEEndEventInf;
import jCompute.Scenario.EndEvents.ScenarioAllPreyLTEEndEventInf;
 ...
 * handles movement , creation and destruction of Agents. and drawing of the
 * agents.
 */
public class SimpleAgentManager implements ScenarioAllPredatorsLTEEndEventInf, ScenarioAllPreyLTEEndEventInf
{
	...
	private ArrayList<SimpleAgent> doList;
	private ArrayList<SimpleAgent> doneList;
	...
	private void addAgents(WorldInf world, SimpleAgentSetupSettings settings)
	{
		...
		for (int i = 0; i < settings.getInitalNumbers(); i++)
		{
			...
			addNewAgent(new SimpleAgent(world, 0, x, y, new SimpleAgentStats(new SimpleAgentType(settings.getType()),
					settings.getColor(), settings.getSpeed(), settings.getSize(), settings.getStartingEnergy(), 100f,
					settings.getHungerThres(), settings.getViewRange(), settings.getMoveCost(),
					settings.getReproductionCost(), settings.getConsumptionRate(), settings.getDigestiveEfficiency(),
					settings.getREDiv())));
		}
	...
	/**
	 * Add an agent to the done list for the next step - keeps counts of agents
	 * and types added.
	 * 
	 * @param agent
	 */
	...
	/**
	 * Add an NEW agent to the done list for the next step and gives it a UID -
	 * keeps counts
	 * 
	 * @param agent
	 */
	...
	public void doStep(KNNInf<GenericPlant> plantKDTree)
	{
		...
		for (int a = 0; a < size; a++)
		{
										...
										// be empty.. (Heap exception avoidance)
			{
				...
				/*
				 * calculate if the Nearest Plants are in the view Range of the
				 * current agent
				 */
	...
	/**
	 * Pseudo Code : If the distance between the closest agents surface is less
	 * than the view range of the current agent then it is in view True - add it
	 * to the Agent to the current agents view. False- clear the view.
	 */
	private void agentViewRangeKDSQAgents(SimpleAgent currentAgent, SimpleAgent nearestAgent)
	{
		...
		/*
		 * System.out.print("Ax "+ currentAgent.body.getBodyPos().getX()+ " Ay "
		 * +currentAgent.body.getBodyPos().getY()); System.out.print(":");
		 * System.out.print("Px "+ nearestAgent.body.getBodyPos().getX()+ " Py "
		 * +nearestAgent.body.getBodyPos().getY()); DebugLogger.output("");
		 * DebugLogger.output("Dis : " + dis); DebugLogger.output("Radius : " +
		 * radiusSize);
		 */
	...
	/**
	 * As above but for plants - with the logical exception that plants can be
	 * extinct and thus the list empty.
	 */
	private void agentViewRangeKDSQPlants(SimpleAgent currentAgent, GenericPlant nearestPlant)
	{
		 ...
		 * System.out.print("Ax "+ currentAgent.body.getBodyPos().getX()+ " Ay "
		 * +currentAgent.body.getBodyPos().getY()); System.out.print(":");
		 * System.out.print("Px "+ nearestPlant.body.getBodyPos().getX()+ " Py "
		 * +nearestPlant.body.getBodyPos().getY()); DebugLogger.output("");
		 * DebugLogger.output("Dis : " + dis); DebugLogger.output("Radius : " +
		 * radiusSize);
		 */
	...
	/*
	 * Optimised method Integrates agent step, Agent Reproduction Next step
	 * kd-tree generation and Stat collection
	 */
	private void part2()
	{
		agentKDTree = new thirdGenKDWrapper<SimpleAgent>(2);
		...
		for (int a = 0; a < size; a++)
		{
			...
			if (!temp.body.stats.isDead())
			{
				...
				{
					// DebugLogger.output("Agent temp.body.stats.canReproduce()"
					// + temp.body.stats.canReproduce() );
					 ...
					 * stats here.
					 */
					addNewAgent(new SimpleAgent(world, 0, temp.body.getBodyPos().getX() + 0.01f, temp.body.getBodyPos()
							.getY() - 0.01f, new SimpleAgentStats(new SimpleAgentType(temp.body.stats.getType()
							.getType()), temp.body.stats.getColor(), temp.body.stats.getMaxSpeed(),
							temp.body.stats.getSize(), temp.body.stats.getStartingEnergy(), 100f,
							temp.body.stats.getHungryThreshold(), temp.body.stats.getBaseViewRange(),
							temp.body.stats.getBaseMoveCost(), temp.body.stats.getBaseReproductionCost(),
							temp.body.stats.getEnergyConsumptionRate(), temp.body.stats.getDigestiveEfficency(),
							temp.body.stats.getReproductionEnergyDivision())));

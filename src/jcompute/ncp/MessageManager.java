...
import jcompute.ncp.NCPDefinitions.Timeout;
...
import jcompute.ncp.message.control.NodeOrderlyShutdownReply;
import jcompute.ncp.message.control.NodeOrderlyShutdownRequest;
import jcompute.ncp.message.monitoring.ActivityTestReply;
import jcompute.ncp.message.monitoring.ActivityTestRequest;
...
public class MessageManager
{
	...
	public boolean start(int txFreq)
	{
		...
		try
		{
			setTimeOut(NCPDefinitions.Timeout.ReadyState);
		}
		...
		try
		{
			...
			while(true)
			{
				...
				switch(type)
				{
					...
					break;
					case NCPDefinitions.ActivityTestRequest:
					{
					...
					break;
					case NCPDefinitions.ActivityTestReply:
					{
	...
	private boolean needsActivityTest()
	{
		...
		boolean needsTest = (timeout == NCPDefinitions.Timeout.Inactivity) ? ((System.currentTimeMillis() - lastActivityTestMessageTime) >= NCPDefinitions.ActivityTestFreq) : false;

...
import jcompute.ncp.NCPDefinitions.ProtocolMode;
import jcompute.ncp.NCPDefinitions.ProtocolState;
...
import jcompute.ncp.message.registration.ConfigurationAck;
import jcompute.ncp.message.registration.ConfigurationRequest;
import jcompute.ncp.message.registration.RegistrationReqAck;
import jcompute.ncp.message.registration.RegistrationReqNack;
import jcompute.ncp.message.registration.RegistrationRequest;
...
public class NCPSocket implements Closeable
{
	...
	public boolean connect(String address, int socketRXBuffer, int socketTXBuffer, boolean tcpNoDelay, int txFreq)
	{
		...
		log.info("Connecting to : " + address + "@" + NCPDefinitions.StandardServerPort);
		...
		try
		{
			...
			socket.connect(new InetSocketAddress(address, NCPDefinitions.StandardServerPort), 5000);
	...
	final int runs)
	{
		...
		while(true)
		{
			...
			if(message == null)
			{
				...
				log.error(NCPDefinitions.DisconnectReason.NullNCPRecv);
			...
			if(isReadyStateTimeOut())
			{
				...
				log.error(NCPDefinitions.DisconnectReason.ReadyStateTimeout);
			...
			{
				case NCPDefinitions.RegReq:
				{
					// Invalid Sequence Check - we must in the ProtocolState.CON
					if(ncpState != ProtocolState.CON)
					{
						// Log the state and message type
						log.error(NCPDefinitions.DisconnectReason.InvRegSeq.appendfromStateInfoAndLastNCPType(ncpState, message.getType()));
					...
					if(req.getProtocolVersion() != NCPDefinitions.NCP_PROTOCOL_VERSION)
					{
						ncpMM.sendRegistrationReqNack(NCPDefinitions.ProtocolVersionMismatch, NCPDefinitions.NCP_PROTOCOL_VERSION);
						
						// Log the state and message type
						log.error(NCPDefinitions.DisconnectReason.NCPVersionMismatch.appendfromStateInfoAndLastNCPType(ncpState, message.getType()));
				...
				case NCPDefinitions.RegAck: // TODO move proto version
				{
					...
					{
						// Log the state and message type
						log.error(NCPDefinitions.DisconnectReason.RegAckUidMismatch.appendfromStateInfoAndLastNCPType(ncpState, message.getType()));
				...
				case NCPDefinitions.RegNack:
				{
					...
					switch(reason)
					{
						...
						case NCPDefinitions.ProtocolVersionMismatch:
							
							log.error("Registration negative acknowledgement : Protocol Version Mismatch");
							log.error("Local " + NCPDefinitions.NCP_PROTOCOL_VERSION + " Remote " + value);
					...
					log.warn(NCPDefinitions.DisconnectReason.NCPNack.appendfromStateInfoAndLastNCPType(ncpState, message.getType()));
				...
				{
					// Log the state and message type
					log.error(NCPDefinitions.DisconnectReason.UnknownNCPType.appendfromStateInfoAndLastNCPType(ncpState, message.getType()));
		...
		if(ncpState != ProtocolState.RDY)
		{
			...
			shutdown(NCPDefinitions.DisconnectReason.RegNotCompleted.toString());
		...
		try
		{
			...
			ncpMM.setTimeOut(NCPDefinitions.Timeout.Inactivity);
		}
	...
	public NCPMessage getReadyStateMessage()
	{
		...
		if(ncpState != ProtocolState.RDY)
		{
			...
			shutdown(NCPDefinitions.DisconnectReason.NCPNotReady.appendfromStateInfo(ncpState));
		...
		if(message == null)
		{
			...
			if(!ncpMM.isConnected())
			{
				...
				shutdown(NCPDefinitions.DisconnectReason.TCPDisconnected.appendfromStateInfo(ncpState));
			}
		...
		if((type == NCPDefinitions.RegReq) || (type == NCPDefinitions.RegAck) || (type == NCPDefinitions.RegNack) || (type == NCPDefinitions.ConfReq) || (type == NCPDefinitions.ConfAck))
		{
			// Invalid to register again when in the ready state.
			shutdown(NCPDefinitions.DisconnectReason.InvRegSeq.appendfromStateInfoAndLastNCPType(ncpState, type));
	...
	{
		return NCPDefinitions.Timeout.ReadyState.hasTimeoutError((int) (System.currentTimeMillis() - ncpStartTime));
	}
}
...
public class JComputeProgressBar extends JComponent
{
	...
	public JComputeProgressBar(Font font, Color selectedColor)
	{
		...
		barFills = new Color[]
		{
			ColorConstants.BabyBlue.brighter().brighter(), ColorConstants.BabyBlue.brighter(), ColorConstants.BabyBlue
		};
	...
	protected void paintComponent(Graphics g1)
	{
		...
		if(getWidth() > (pad * 8))
		{
			...
			{
				// This will NOT be correct if the height changes - OK for fixed height rows.
				linearGradientPaint = new LinearGradientPaint(0, pad, 0, getHeight() - ((pad * 2) - 1), new float[]
				{
					0.165f, 0.495f, 0.825f
				}, barFills);
			}
			
			// Progress Bar
			g2.setPaint(linearGradientPaint);
			g2.fillRect(pad, pad, (int) ((getWidth() - (pad * 2)) * barWidth), getHeight() - (pad * 2) - 1);

...
public class AboutWindow extends JDialog
{
	...
	public AboutWindow()
	{
		...
		gridPanel.changeValText(buildDate, jcInfo[0]);
		gridPanel.changeValText(buildRev, jcInfo[1]);
		gridPanel.changeValText(launchIndex, jcInfo[2]);
	...
	private String[] retrieveJComputeBuildInfo()
	{
		String val[] =
		{
			"Error Retrieving Build Information", "Error Retrieving Build Information", "Error Retrieving Build Information"
		};
		...
		try
		{
			...
			val[1] = info.getBuildRevison();
			val[2] = info.getLaunched();
		}
		catch(IOException e)
		{
			e.printStackTrace();
	...
	private String[] retrieveOSInfo()
	{
		...
		return new String[]
		{
			osInfo.getOSName(), osInfo.getSystemArch(), Integer.toString(osInfo.getHWThreads()), Integer.toString(osInfo.getSystemPhysicalMemorySize())
		};
	}

...
public class KDTreeBulk
{
	...
	public void load(ArrayList<KNNFloatPosInf> listIn)
	{
		...
		// Root Node
		rootnode = new KDNode<KNNFloatPosInf>(0, list.get(pivot).getXY(), list.get(pivot));
	...
	private KDNode<KNNFloatPosInf> split(int depth, KDNode<KNNFloatPosInf> parent, List<KNNFloatPosInf> list)
	{
		...
		KDNode<KNNFloatPosInf> node = new KDNode<KNNFloatPosInf>(0, list.get(pivot).getXY(), list.get(pivot));
	...
	{
		int depth = 0;
		System.out.println("Depth : " + depth + " pos " + rootnode.getPos());
	...
	private void dumpChildern(int depth, KDNode<KNNFloatPosInf> parent)
	{
		...
		if(left != null)
		{
			System.out.println("Depth : " + depth + " pos " + left.getPos());
		}
		
		KDNode<KNNFloatPosInf> right = parent.getRightChild();
		if(right != null)
		{
			System.out.println("Depth : " + depth + " pos " + right.getPos());
		}
